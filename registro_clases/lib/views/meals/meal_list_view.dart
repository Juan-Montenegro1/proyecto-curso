...
class _MealListViewState extends State<MealListView> {
  ...
  Future<void> _search() async {
    ...
    debugPrint('MealListView: iniciando búsqueda para "${_searchCtrl.text.trim()}"');
    try {
      final results = await _service.searchMeals(query: _searchCtrl.text.trim());
      debugPrint('MealListView: resultados obtenidos: ${results.length}');
      if (!mounted) return;
      setState(() {
        _meals = results;
        _isLoading = false;
      ...
      debugPrint('MealListView: excepción en búsqueda: $e');
      if (!mounted) return;
      setState(() {
        _error = e.toString();
        _isLoading = false;
